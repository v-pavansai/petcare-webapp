...
async def analyze_health(request: AnalyzeRequest, current_email: str = Depends(get_current_email)):
    ...
    prompt = (
        ...
        "STEP 1 - VALIDATION (do this first, internally, and be strict/skeptical, not charitable):\n"
        ...
        "- Check if the symptom text describes a REAL, medically coherent symptom or behavior that the "
        "stated species can actually exhibit. Do NOT proceed to diagnosis if the text is:\n"
        "  * gibberish, empty, or random characters\n"
        "  * a fictional/cartoon ability no real animal has (e.g. 'shoots lightning')\n"
        "  * biologically nonsensical or physically impossible for that species (e.g. a bird 'can't produce "
        "water' - animals don't produce water; a fish 'can't fly') - treat these as invalid, not as a real "
        "clinical sign, even if it sounds superficially medical\n"
        "  * vague, ambiguous, or so underspecified that a real vet could not act on it (e.g. 'something is "
        "wrong', 'he is different today')\n"
        "  * describing normal/expected behavior for the species (e.g. a parrot not talking yet, a cat sleeping a lot)\n"
        "- If you find yourself having to guess, reinterpret, or charitably 'translate' what the user might "
        "have meant into a real symptom, that is a sign the input is NOT valid - do not proceed to diagnosis.\n"
        "- Only proceed to CASE C if the symptom is a specific, plausible, real-world clinical sign or "
        "behavior change for that species (e.g. loss of appetite, lethargy, vomiting, limping, feather "
        "plucking, labored breathing).\n"
        ...
        "CASE A - Irrelevant, implausible, or unusable input:\n"
        "Output only: 'Status: Unable to Assess' followed by one line explaining why (e.g. 'The uploaded "
        "photo does not appear to show a pet.' or 'The symptom described is not a recognized clinical sign "
        "for this species - please re-describe what you're observing.'). Ask the user to re-upload a clear "
        "photo and/or re-describe symptoms in concrete, observable terms. Do not output a severity assessment.\n\n"
        ...
        "CASE C - Valid photo and/or valid, plausible symptoms indicating a possible health concern:\n"
